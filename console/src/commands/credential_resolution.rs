...
mod tests {
	...
	/// Tracing layer that copies every event's fields into the
	/// shared capture buffer.
	struct CapturingLayer {
		captured: std::sync::Arc<CapturedFields>,
	}
	...
	impl<S: tracing::Subscriber> tracing_subscriber::Layer<S> for CapturingLayer {
		fn register_callsite(
			...
			// Force per-event evaluation. Without this, parallel
			// tests can prime tracing's callsite cache with a
			// dispatcher that has no interest in these events,
			// after which `with_default` doesn't re-evaluate and
			// the capture buffer stays empty.
	...
	fn resolved_values_never_appear_in_tracing_output() {
		...
		// `with_default` scopes the dispatcher to the current
		// thread; the guard restores the previous dispatcher on
		// scope exit. A current-thread runtime ensures the future
		// polls on this thread, which is the one carrying the
		// dispatcher — a multi-thread runtime would poll on
		// worker threads where the dispatcher is not set.
		tracing::subscriber::with_default(subscriber, || {
			...
			// Confirm the resolution actually saw the sentinel,
